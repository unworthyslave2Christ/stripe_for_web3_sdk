...
export async function subscribe({
  ...
  if (!Number.isInteger(plan.merchantId) || plan.merchantId <= 0) {
    throw new Error("Invalid merchant ID.");
  }

  if (!plan.paymentToken) {
    throw new Error("Billing plan payment token is missing.");
  }

  if (BigInt(plan.amount) <= 0n) {
    throw new Error("Billing plan amount must be greater than zero.");
  }

  if (plan.status !== "ACTIVE") {
    throw new Error("Billing plan is not active.");
  }
  ...
  const [ownerWallet] = await client.walletClient.getAddresses();

  if (!ownerWallet) {
    throw new Error("No connected customer wallet was found.");
  }
   ...
   */
  const { customer, kernelAccount, kernelClient, permission, permissionId } =
  ...
  if (!customer) {
    throw new Error("Customer was not returned by the backend.");
  }
  ...
  if (!permissionId) {
    throw new Error("Customer billing permission id is missing.");
  }
  ...
  if (
    ...
    throw new Error("Recovered Kernel does not belong to the customer.");
  }
  ...
  const balance = await client.publicClient.readContract({
    address: plan.paymentToken as Address,
  ...
  if (BigInt(balance as bigint) < BigInt(plan.amount)) {
  ...
  const activeSubscription = await findActiveSubscription({
    ...
    subscriber: customer.smartAccount,
   ...
   */
  const data = encodeBillingProtocolCall("subscribe", [
    BigInt(plan.planId),
  ...
  const mirrored = (await mirror({
    ...
    body: {
      ...
      transactionHash: extractTransactionHash(receipt) ?? userOperationHash,
  ...
  const normalizedSubscription = normalizeSubscription(
    mirrored.subscription ?? mirrored ?? subscription,
  );
...
async function findActiveSubscription({
  ...
  const subscriptionIds = (await client.publicClient.readContract({
    address: client.contractAddress,

    abi: billingProtocolReadAbi,
  ...
  for (const subscriptionId of subscriptionIds) {
    const subscription = (await client.publicClient.readContract({
      address: client.contractAddress,
    ...
    const subscriptionSubscriber =
      subscription?.subscriber ?? subscription?.[1];

    const status = subscription?.status ?? subscription?.[6];

    if (
      subscriptionSubscriber &&
      subscriptionSubscriber.toLowerCase() === subscriber.toLowerCase() &&
      isActiveSubscriptionStatus(status)
    ) {
      return normalizeOnChainSubscription(subscription, subscriptionId, planId);
    }
  }
...
function normalizeOnChainSubscription(
  ...
  return {
    subscriptionId: Number(
      input?.subscriptionId ?? input?.[0] ?? subscriptionId,
    ),

    planId: Number(input?.planId ?? input?.[1] ?? planId),

    customerId: Number(input?.customerId ?? input?.[2] ?? 0),

    subscriber: input?.subscriber ?? input?.[3],

    status: input?.status ?? input?.[6] ?? "ACTIVE",
  };
}
...
function normalizeSubscription(input: any): SubscriptionRecord {
  console.log("shape of input received: ", input);
  return {
    subscriptionId: Number(input.subscriptionId ?? input.subscription_id),
    ...
    nextBillingTime: normalizeDate(
      input.nextBillingTime ?? input.next_billing_time,
    ),

    lastChargedAt:
      (input.lastChargedAt ?? input.last_charged_at)
        ? normalizeDate(input.lastChargedAt ?? input.last_charged_at)
        : undefined,

    cancelledAt:
      (input.cancelledAt ?? input.cancelled_at)
        ? normalizeDate(input.cancelledAt ?? input.cancelled_at)
        : undefined,

    createdAt: normalizeNullableDate(input.createdAt ?? input.created_at),

    updatedAt: normalizeNullableDate(input.updatedAt ?? input.updated_at),

    transactionHash: input.transactionHash ?? input.transaction_hash ?? "0x",

    permissionId: input.permissionId ?? input.permission_id ?? "0x",
...
function extractTransactionHash(receipt: any): `0x${string}` | undefined {
   ...
   */
  return (receipt?.receipt?.transactionHash ?? receipt?.transactionHash) as
    | `0x${string}`
    | undefined;
}
...
async function approveTokenIfNeeded({
  ...
  const approvalData = await encodeErc20Approve(token, spender, amount);
